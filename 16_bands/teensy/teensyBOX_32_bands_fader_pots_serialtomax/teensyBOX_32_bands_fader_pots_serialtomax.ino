...
int pot_smooth_gain;
int pot_smooth_scale;

float pot_filter = 0.25;

int pot_previous_gain;
int pot_previous_scale;
...
void read_pots_serial_to_max()
{
  ...
  pot_smooth_scale = pot_smooth_scale * (1.0 - pot_filter) + pot_scale * pot_filter;

  /*
  Serial.print(pot_gain);
  Serial.print("  ");
  Serial.print(pot_smooth_gain);
  Serial.print("  ||  ");
  Serial.print(pot_scale);
  Serial.print("  ");
...
void serial_read()
{
  if (Serial.available())
  {
    ...
    {
      band_volume[i] = constrain(Serial.read(), 0, 255);
    }
  }
}
